...
### 🔴 Service Management - CRITICAL RULE

**NEVER manually start Maymay backend** — it's managed by systemd:
```bash
# ❌ WRONG! Creates conflict with systemd
cd /home/yongyue/maymay-backend && source env/bin/activate && python app/main.py &
```

**ALWAYS use systemd to restart:**
```bash
# ✅ CORRECT
systemctl restart maymay-backend.service
```

**Why:** Manual processes bind ports (8000) and prevent systemd from starting, causing repeated failures. See `MEMORY.md` → "Service Management" for full details.
